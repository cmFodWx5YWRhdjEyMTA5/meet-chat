...
//controllers
freeChat.controller('Messages', function($scope, $rootScope, $timeout, $ionicScrollDelegate, $ionicModal, $ionicPopup, system) {
	...
	$scope.sendMessage = function() {
		...
		if (system.adCount == system.maxAdCount - 1)
			...
			//AdMob.prepareInterstitial( {adId:system.interstitial, autoShow:false} );	
	...
	$scope.showProfile = function (message) {
		...
		$scope.hasPhoto = true;
		if ( (message.ThumbName === system.dom1 + system.dom2 + "/ify/images/female_icon.png") || (message.ThumbName === system.dom1 + system.dom2 + "/ify/images/man_icon.png") ) 
	...
	$scope.removePhoto =  function () {
		...
		if (c)
		{
			var _ThumbName = (system.profile.gender == 'Female') ? system.dom1 + system.dom2 + '/ify/images/female_icon.png': system.dom1 + system.dom2 + '/ify/images/man_icon.png';
	...
	$scope.report_user = function(user) {
		...
		var str = "Report User! You can report inappropriate profiles, contains nudity or pornographic photos.";
		var c = confirm(str);
		if (c){
			system.showLoading("Thanks, sending email...");
			var result = system.send_report_email(user);
			result.then(function (state) {				
				if (state){
					cordova.plugin.pDialog.dismiss();
					alert("Thanks for your feedback email!");
				}
			});
